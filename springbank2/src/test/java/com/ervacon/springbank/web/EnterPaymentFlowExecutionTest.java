...
public class EnterPaymentFlowExecutionTest extends AbstractXmlFlowExecutionTests {
	...
	private LocalAccount debitAccount = new LocalAccount(new Client("Foo", "Bar", null), "Test-0");
	private Beneficiary creditAccount = new Beneficiary("Credit", "Test-1");
	...
	public void testStartFlow() {
		expect(accountService.getAccounts(0L)).andReturn(Collections.<Account>singletonList(debitAccount));
	...
	public void testSelectDebitAccount() {
		...
		expect(accountService.getAccount(debitAccount.getNumber())).andReturn(debitAccount);
		replay(accountService);
	...
	public void testSubmit() throws Exception {
		...
		paymentProcessingEngine.transfer((Payment)getFlowExecution().getConversationScope().get("payment"));
